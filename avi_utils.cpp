...
typedef struct{
    ...
    WAVEFORMATEX    wfx;        // as given to CreateAvi (.nChanels=0 if none was given).
                                //  Used when audio stream is first created.
    ...
    IAVIStream      *ps;        // video stream, when first created
    IAVIStream      *psCompressed;
    unsigned long   nframe;     //which frame will be added next
    unsigned long   nsamp;      //which sample will be added next
